...
export default function BrandSelect({handleInputChange,brand}) {
  ...
  const allBrands = useSelector((state)=>state.adminProducts['allBrands']);
  ...
  return (
    <Box sx={{ minWidth: 120 }}>
      <FormControl fullWidth>
        ...
        >
           <MenuItem key={'011'} value={"new"}>{"Add New"}</MenuItem>
          {
          allBrands && allBrands.map((obj,key) =>  (<MenuItem key={key} value={obj}>{obj}</MenuItem>))  
        }
        </Select>
